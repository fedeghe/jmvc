...
JMVC.extend('test', {
	'vars' : {
		...
		'testsPassed' : 0,
		'totalTests' : 0,
		'css_path' : [JMVC.vars.baseurl,  'app', 'test', 'css'].join(JMVC.US),
	...
	//
	'init' : function(){
		JMVC.require('sniffer', 'fx', 'lib/shl');
	...
	'testException' : function(testName, code, expectedError){
		...
		*/
		...
		var res = true, i = 0, debuginfo = false;
	...
	'testValue' : function(testName, fn, expectedValue, options){
		'use strict';
		var res = true, i=0, debuginfo = false,real;
		options = options || {};
		JMVC.test.startTest(testName);
