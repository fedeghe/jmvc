JMVC.controllers.demo = function() {
	...
	this.action_index = function () {
		...
		var content = '<h3 id="samples" class="round8 roundbottom">Some JMVC based samples <span id="size"></span></h3>',
			...
			links = {
				...
				'Console': 'console',
				//'get gravatar FS (esc to exit) goo.gl' : '?jmvcgoto=' + encodeURIComponent('http://goo.gl/dXBx9v'), //
				//'get gravatar FS (esc to exit) goo.gl' : '?jmvcgoto=' + encodeURIComponent('http://goo.gl/xzVldj'),
	...
	this.action_logo = function() {
		...
		var M = Math,
			v = JMVC.getView('vacuum');
		v.set({
			'style': 'font-family:verdana;margin:0 auto;width:461px;height:123px;margin-top:80px;position:relative;' + (JMVC.p.back ? 'background-image:url(http://www.jmvc.dev/media/img/jmvc_m1.png);background-color:white;' : ''),
			'content': '&nbsp;',
			'id': 'extralogo'
		});
