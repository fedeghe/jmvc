JMVC.controllers.widget = function () {
	this.index = function () {
		...
		JMVC.require('widget/modal/modal', 'widget/slider/slider', 'widget/accordion/accordion', 
			'form', 'event_scroll'
		);
		...
		var M = Math,
			...
			that = this;
		//
		v.set({'style' : 'font-family:verdana; margin:0 auto; margin-top:10px', 'content' : '&nbsp;', 'id' : 'content'});
		//
		...
		v.render({cback : function () {
			...
			JMVC.events.bind(JMVC.dom.find('#openmodal'), 'click', function () {
				...
				JMVC.modal.open(t, 'Titolo del modal', 800, 80, {bgcolor:'#777', shadow : true});
			});
			...
			JMVC.form.slidenum(inputxt, 10, {from:-100, to:1000});
			...
			//slider
			var slider1 = new JMVC.widget.Slider(JMVC.dom.add(container, 'div', {id:'slider1', style:'margin-top:10px'})),
			...
			JMVC.dom.html(out1, slider1.getValue()['up']);
			...
			var acc = new JMVC.widget.Accordion(150, true);
			...
			acc.addElement('prova9', '<h2>contenuto</h2><p>testo di prova 9</p>');
