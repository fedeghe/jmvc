...
JMVC.widget.Accordion = function (h, slide) {
...
JMVC.widget.Accordion.prototype.render = function (container, open_index, rem) {
	...
	var ul = JMVC.dom.create('ul', {'class' : 'accordionContainer'}),
		attrs,
		self = this,
	...
	//console.debug('open_index (out) : '+open_index)
	JMVC.cookie.set(cookie_rem_name , open_index);
	
	
	JMVC.dom.append(container, ul);
	
	for(var i = 0, len = this.els.length, li = false; i < len; i += 1) {
		li = JMVC.dom.add(ul, 'li', {'class' : 'accordionElement'});
		attrs = {'class' : 'content', 'style':'minHeight:' + this.height + 'px'};
		...
		this.els[i].dom = [
			JMVC.dom.create('div', {'class' : 'title' + (i == open_index ? ' selected' : '')}, this.els[i].title),
		...
		(function (j) {
			...
			JMVC.events.bind(
				...
				function (e) {
					...
					//console.debug(cookie_rem_name, y, self.els[y])
