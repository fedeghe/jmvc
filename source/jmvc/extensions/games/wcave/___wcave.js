...
!function (W) {
    ...
    */
    ...
    var Wcave = function () {},
        ...
        conf = {
            ...
            // audio file
            audiointro : {
                file: JMVC.vars.baseurl + '/media/audio/revolve',
                formats: {
                    mp3: 'audio/mpeg',
                    ogg: 'audio/ogg'
                }
    ...
    */
    ...
    Wcave.prototype.init = function () {
        ...
        cache.panelBlink = W.setInterval(function () {
    ...
    Wcave.prototype.gameoverize = function () {
        vars.active = false;
        vars.gameover = true;
        return;
    }
     ...
     * @return {[type]} [description]
     */
    Wcave.prototype.drawPills = function drawPills() {
        var i,
            l;
        self.checkFeverAlive();
        self.clearPpills();
        function drawPill(x, y, tpill) {
            self.ctx.beginPath(
                x,
                y
            );
     ...
     * @return {[type]} [description]
     */
    Wcave.prototype.gameOver = function gameOver() {
    ...
    Wcave.prototype.utils = {
        ...
        formatScore : function formatScore(s) {
            s = String(s);
            var ret = [],
                l = s.length - 1,
                cnt = 0;
        ...
        orderJsonBy : function orderJsonBy(j, field) {
            var res = {};
        },
        stilize : function stilize(el, rules) {
            var j;
            for (j in rules) {
        ...
        makeNS : function makeNS(str, obj, ctx) {
            var self = this,
                chr = '.',
                els = str.split(chr),
                u = 'undefined',
        ...
        unbind : function unbind(el, tipo, func) {
            if (el === null) {return; }
        ...
        replaceAll : function replaceAll(tpl, o, dD, Dd) {
        ...
        checkpound : function checkpound(color) {
            return (color.charAt(0) === '#') ? color.substr(1) : color;
        },
        getRandomColor : function getRandomColor() {
            return '#' + (Math.random() * 0xFFFFFF << 0).toString(16);
        },
        getGradientArray : function getGradientArray(color1, color2, num) {
            var ret = [],
                int1,
                int2,
                diffs,
        ...
        padme : function padme(val, el, pos, lngt) {
            var len = lngt || 2;
        ...
        pdistance : function pdistance(p1x, p1y, p2x, p2y) {
            return Math.max(Math.abs(p2x - p1x), Math.abs(p2y - p1y));
            //return Math.sqrt((p2x - p1x) * (p2x - p1x) + (p2y - p1y) * (p2y - p1y));
        },
        out : function out(msg, what) {
