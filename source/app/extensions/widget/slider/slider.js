...
JMVC.widget.Slider.prototype.init = function (bounds, top, down, step) {
    ...
    function manageClick (e) {
        var t = e.target,
            vReal = e.layerX || e.offsetX, // relative to the container, outerbar, innerbar (relative to 1 cur), opera uses offsetX
            vVirt = that.real2virt(vReal),
            vals = that.getValue(),
        ...
        if (t === that.node) {
            ...
            if (vVirt < vals.down) {
                ...
                val = vReal > that.min ? vReal - that.cursor_width_middle - 2 : 0;
            } else {
                node = that.cursor_max;
                val = vReal > that.max ? that.max - that.cursor_width_middle : vReal - that.cursor_width_middle - 1;
            }
            ...
            if (that.cursor_min) {
                ...
                val = vReal + rvals.down;// + that.cursor_width_middle;
                node = vReal < mean ? that.cursor_min : that.cursor_max;
            } else { // only max
                val = vReal - that.cursor_width_middle - 1;
                node = that.cursor_max;
            }
        }
    ...
    // innerbar
    innerbar = this.innerbar = document.createElement('div');
